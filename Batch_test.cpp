...
int main(int argc, char* argv[]) {
  ...
  //Initial pose
    ...
    //Vector4 current_rotation3 = imuapproximate(gps_measurements[2].time, imu_measurements);
    //auto current_pose_global3 = Pose3(Rot3(Quaternion(current_rotation3[3], current_rotation3[0], current_rotation3[1], current_rotation3[2])), gps_measurements[2].position);
    ...
    auto sigma_init_x = noiseModel::Diagonal::Precisions((Vector6() << Vector3::Constant(0),	//0
                                                                       Vector3::Constant(1e-6))
                                                         .finished());
    ...
    for (size_t i = first_gps_pose; i < gps_measurements.size() - 1; i++) {
        ...
        if (i == first_gps_pose) {
            ...
            //auto current_pose_key0= X(i-2);
            //auto current_vel_key0 = V(i-2);
            //auto current_bias_key0 = B(i-2);
            auto current_pose_key1= X(i);
            auto current_vel_key1 = V(i);
            auto current_bias_key1 = B(i);
            //new_values.insert(current_pose_key0, current_pose_global);
            //new_values.insert(current_vel_key0, current_velocity_global2);
            //new_values.insert(current_bias_key0, current_bias);
            ...
            new_values.insert(current_pose_key, current_pose_global2);
            new_values.insert(current_vel_key, current_velocity_global);
            new_values.insert(current_bias_key, current_bias);
            
            //cout << "current_pose_global: " << current_pose_global << endl;
            //cout << "current_pose_globa2l: " << current_pose_global2 << endl;
            //cout << "current_pose_global3: " << current_pose_global3 << endl;

            //new_factors.emplace_shared<PriorFactor<Pose3>>(current_pose_key0, current_pose_global, sigma_init_x);
            //new_factors.emplace_shared<PriorFactor<Vector3>>(current_vel_key0, current_velocity_global2, sigma_init_v);
            //new_factors.emplace_shared<PriorFactor<imuBias::ConstantBias>>(current_bias_key0, current_bias, sigma_init_b);
